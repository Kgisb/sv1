...
with tabs[7]:
    ...
    if missing_cols:
        ...
        # Safe Top-N selector (handles small number of campaigns and min==max)
        _max_n = int(max(1, len(camp_f)))
        if _max_n <= 6:
            # For very small sets, allow 1..N to avoid min==max edge-cases
            top_n = st.slider("Show top N campaigns by revenue", 1, _max_n, value=min(15, _max_n), key="mp_top_n")
        else:
            _hi = int(min(50, _max_n))
            _lo = 5
            if _lo >= _hi:
                top_n = _hi  # no slider needed
            else:
                top_n = st.slider("Show top N campaigns by revenue", _lo, _hi, value=min(15, _hi), key="mp_top_n")
        else:
            top_n = st.slider("Show top N campaigns by revenue", 5, min(50, _max_n), value=min(15, _max_n), key="mp_top_n")
        top = camp_f.sort_values("Revenue_INR", ascending=False).head(top_n).copy()
