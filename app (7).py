...
def aggregate_revenue_trend(d: pd.DataFrame, date_col: str, granularity: str) -> pd.DataFrame:
    '''
    Aggregates Total_Value_INR by selected granularity using date_col.
    granularity: 'Day', 'Week', 'Month'
    Returns DataFrame indexed by period label with a single column Total_Value_INR.
    '''
    ...
    trend_tbl = (
        t.groupby("period")["Total_Value_INR"]
        .sum()
        .reset_index()
        .sort_values("period")
        .set_index("period")
...
with tabs[0]:
    ...
    if date_col is None:
        ...
        granularity = st.selectbox(
            ...
            index=0,
            help="Choose how to aggregate revenue over time."
        )

        trend_tbl = aggregate_revenue_trend(df_f, date_col, granularity)
