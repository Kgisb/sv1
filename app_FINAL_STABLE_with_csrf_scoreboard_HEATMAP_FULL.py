...
**Productivity Index** = **0.6 × (Inquiries per Active Day)** + **0.4 × (Inquiries per Tenure Month)**
                ...
                if prod_df is None or prod_df.empty:
                    ...
                    # ---------------------------
                    # Sales Rep Comparative Rank Table (CSRF Scoreboard)
                    # ---------------------------
                    st.subheader("Sales Rep Comparative Rank Table (CSRF Scoreboard)")
                    st.caption("Rank 1 = best. Conversion% = Enrollments ÷ Leads. Aging = Tenure (months) from DOJ/master map.")
                    
                    score = selected_table.rename(columns={"Group":"CSRF"}).copy()
                    ...
                    if "Total_Dials" in df_f.columns and "Sales_Rep" in df_f.columns:
                        ...
                        calls = tmpc.groupby("Sales_Rep")["Total_Dials"].mean().reset_index().rename(columns={"Sales_Rep":"CSRF","Total_Dials":"Avg_Calls_Made"})
                        score = score.merge(calls, on="CSRF", how="left")
                    else:
                    ...
                    if "Sales_Rep" in df_f.columns:
                        if lead_id:
                            leads_tbl = df_f.groupby("Sales_Rep")[lead_id].nunique(dropna=True).reset_index().rename(columns={"Sales_Rep":"CSRF", lead_id:"Leads"})
                        else:
                            leads_tbl = df_f.groupby("Sales_Rep").size().reset_index(name="Leads").rename(columns={"Sales_Rep":"CSRF"})
                    else:
                        leads_tbl = pd.DataFrame({"CSRF": score["CSRF"], "Leads": np.nan})
                    
                    score = score.merge(leads_tbl, on="CSRF", how="left")
                    ...
                    if "prod_df" in locals() and prod_df is not None and not prod_df.empty and "Productivity_Index" in prod_df.columns:
                        p = prod_df.rename(columns={"Sales_Rep":"CSRF"}) if "Sales_Rep" in prod_df.columns else prod_df.copy()
                        score = score.merge(p[["CSRF","Productivity_Index"]], on="CSRF", how="left")
                    else:
                    ...
                    st.divider()
                    st.subheader("Correlation Heatmap (CSRF Metrics)")
                    ...
                    mcol = st.selectbox('Plot rank metric', metric_cols, index=0, key='csrf_rank_metric')
                    ...
                    st.vega_lite_chart(
                        plot,
                        {
                            ...
                            'mark':'bar',
                            'encoding':{
                                'x':{'field':'CSRF','type':'nominal','sort':'y','axis':{'labelAngle':-45}},
                                'y':{'field':mcol,'type':'quantitative','axis':{'title':mcol}},
                                'tooltip':[{'field':'CSRF','type':'nominal'},{'field':mcol,'type':'quantitative'}]
                            }
                        },
                        use_container_width=True
                    )
