...
with tabs[8]:
    ...
    # --- Business-friendly Accuracy % (derived from MAE) ---
    avg_actual = float(np.mean(np.abs(y))) if len(y) else 0.0
    if avg_actual > 0:
        accuracy_pct = max(0.0, 100.0 - (mae / avg_actual) * 100.0)
    else:
        accuracy_pct = np.nan
    ...
    c4.metric("Accuracy % (MAE-based)", "NA" if pd.isna(accuracy_pct) else f"{accuracy_pct:.1f}%")
