...
def rep_metric_table(df_base: pd.DataFrame, rep_col: str, revenue_series: pd.Series, enroll_series: pd.Series) -> pd.DataFrame:
    ...
    g = g.rename(columns={"_rev":"Revenue", "_enr":"Enrollments"})
    return g

def pick_reps_by_bucket(rep_df: pd.DataFrame, metric: str, bucket: str) -> list:
    # bucket: All, Top 20%, Top quartile, Q2, Q3, Bottom quartile
    rep_df = rep_df.copy()
...
# ---------------------------
# Top "pill" (mode switch) - right aligned
# ---------------------------
top_left, top_right = st.columns([3, 1.2])
with top_right:
    mode = st.radio(
        "Mode",
        options=["Overall", "Sales Rep"],
        horizontal=True,
...
# Prepare base revenue/enroll series (for df_base)
base_revenue = df_base["Total_Value_INR"].copy()
base_enroll  = enroll_series_fn(df_base)

# ---------------------------
# Sales Rep mode filter (only if pill selected)
# ---------------------------
df_f = df_base.copy()
...
if mode == "Sales Rep":
    if "Sales_Rep" not in df_base.columns:
        ...
        # Which metric to rank reps on?
        if view_metric == "Total Revenue":
            rank_metric = "Revenue"
        elif view_metric == "Total Enrollments":
            rank_metric = "Enrollments"
        else:
            rank_metric = "AOV"

        # Controls for rep selection
        with top_left:
            st.caption("Sales Rep mode is ON — choose reps or buckets (based on current left filters).")
        ...
        if sel_type == "Selected reps":
            ...
            reps_in_bucket = pick_reps_by_bucket(rep_df.rename(columns={"Sales_Rep":"Sales_Rep"}), rank_metric, bucket)
            if reps_in_bucket:
        ...
        # Helpful rep table (only in Sales Rep mode)
        with st.expander("Sales Rep ranking table (based on current filters)", expanded=False):
            show = rep_df.copy()
            show["Revenue"] = show["Revenue"].apply(format_inr)
            show["Enrollments"] = show["Enrollments"].astype(int)
            show["AOV"] = show["AOV"].apply(format_inr)
            st.dataframe(show.sort_values(rank_metric, ascending=False), use_container_width=True)

# ---------------------------
# Compute metrics on df_f
# ---------------------------
