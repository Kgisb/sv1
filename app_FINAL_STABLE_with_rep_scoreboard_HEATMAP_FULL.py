...
**Productivity Index** = **0.6 × (Inquiries per Active Day)** + **0.4 × (Inquiries per Tenure Month)**
                ...
                if prod_df is None or prod_df.empty:
                    ...
                    # ---------------------------
                    # Sales Rep Comparative Rank Table (Sales Rep Scoreboard)
                    # ---------------------------
                    st.subheader("Sales Rep Comparative Rank Table (Sales Rep Scoreboard)")
                    st.caption("Rank 1 = best. Conversion% = Enrollments ÷ Leads. Aging = Tenure (months) from DOJ/master map.")
                    
                    score = selected_table.rename(columns={"Group":"Sales Rep"}).copy()
                    score["Revenue"] = pd.to_numeric(score["Revenue"], errors="coerce").fillna(0)
                    score["Enrollments"] = pd.to_numeric(score["Enrollments"], errors="coerce").fillna(0)
                    score["AOV"] = np.where(score["Enrollments"]>0, score["Revenue"]/score["Enrollments"], np.nan)
                    
                    # Aging (Tenure) from master_map
                    age = master_map.copy()
                    if "Sales_Rep" in age.columns:
                        age = age.rename(columns={"Sales_Rep":"Sales Rep"})
                    if "Tenure_Months" in age.columns:
                        age["Tenure_Months"] = pd.to_numeric(age["Tenure_Months"], errors="coerce")
                        age_tbl = age[["Sales Rep","Tenure_Months"]].drop_duplicates()
                    else:
                        if "Date_of_Joining" in age.columns:
                            age["Date_of_Joining"] = pd.to_datetime(age["Date_of_Joining"], errors="coerce")
                            age_tbl = age[["Sales Rep","Date_of_Joining"]].drop_duplicates()
                            age_tbl["Tenure_Months"] = ((pd.Timestamp("today") - age_tbl["Date_of_Joining"]).dt.days/30.0)
                            age_tbl = age_tbl[["Sales Rep","Tenure_Months"]]
                        else:
                            age_tbl = pd.DataFrame({"Sales Rep": score["Sales Rep"], "Tenure_Months": np.nan})
                    
                    score = score.merge(age_tbl, on="Sales Rep", how="left")
                    
                    # Avg Calls Made (Total_Dials) from df_f if available
                    if "Total_Dials" in df_f.columns and "Sales_Rep" in df_f.columns:
                        tmpc = df_f.copy()
                        tmpc["Total_Dials"] = pd.to_numeric(tmpc["Total_Dials"], errors="coerce")
                        calls = tmpc.groupby("Sales_Rep")["Total_Dials"].mean().reset_index().rename(columns={"Sales_Rep":"Sales Rep","Total_Dials":"Avg_Calls_Made"})
                        score = score.merge(calls, on="Sales Rep", how="left")
                    else:
                        score["Avg_Calls_Made"] = np.nan
                    
                    # Leads per rep (LeadID preferred) → Conversion%
                    lead_id = None
                    for cand in ["LeadID","LeadId","Lead_ID","leadid","lead_id","InquiryID","Inquiry_Id","ProspectID","ProspectId"]:
                        if cand in df_f.columns:
                            lead_id = cand
                            break
                    if "Sales_Rep" in df_f.columns:
                        if lead_id:
                            leads_tbl = df_f.groupby("Sales_Rep")[lead_id].nunique(dropna=True).reset_index().rename(columns={"Sales_Rep":"Sales Rep", lead_id:"Leads"})
                        else:
                            leads_tbl = df_f.groupby("Sales_Rep").size().reset_index(name="Leads").rename(columns={"Sales_Rep":"Sales Rep"})
                    else:
                        leads_tbl = pd.DataFrame({"Sales Rep": score["Sales Rep"], "Leads": np.nan})
                    
                    score = score.merge(leads_tbl, on="Sales Rep", how="left")
                    score["Conversion_%"] = np.where(score["Leads"]>0, (score["Enrollments"]/score["Leads"])*100, np.nan)
                    
                    # Productivity (from prod_df if available above)
                    if "prod_df" in locals() and prod_df is not None and not prod_df.empty and "Productivity_Index" in prod_df.columns:
                        p = prod_df.rename(columns={"Sales_Rep":"Sales Rep"}) if "Sales_Rep" in prod_df.columns else prod_df.copy()
                        score = score.merge(p[["Sales Rep","Productivity_Index"]], on="Sales Rep", how="left")
                    else:
                        score["Productivity_Index"] = np.nan
                    
                    def rdesc(s):
                        return s.rank(method='min', ascending=False)
                    
                    rank_tbl = pd.DataFrame({
                        'Sales Rep': score['Sales Rep'],
                        'Rank_Revenue': rdesc(score['Revenue']),
                        'Rank_Enrollments': rdesc(score['Enrollments']),
                        'Rank_AOV': rdesc(score['AOV']),
                        'Rank_Aging': rdesc(score['Tenure_Months']),
                        'Rank_Productivity': rdesc(score['Productivity_Index']),
                        'Rank_Avg_Calls': rdesc(score['Avg_Calls_Made']),
                        'Rank_Conversion_%': rdesc(score['Conversion_%']),
                    }).sort_values('Rank_Revenue')
                    
                    st.dataframe(rank_tbl, use_container_width=True, height=420)
                    st.divider()
                    st.subheader("Correlation Heatmap (Sales Rep Metrics)")
                    st.caption("Correlation between Revenue, Enrollments, AOV, Aging (Tenure), Productivity, Avg Calls, and Conversion%.")
                    
                    # Build correlation base from the same score table
                    corr_base = score[[
                        'Revenue',
                        'Enrollments',
                        'AOV',
                    ...
                    corr_base = corr_base.apply(pd.to_numeric, errors='coerce')
                    corr = corr_base.corr()
                    corr_long = corr.reset_index().melt(id_vars='index', var_name='Metric', value_name='Correlation')
                    ...
                    st.vega_lite_chart(
                        corr_long,
                        {
                            ...
                            'encoding': {
                                ...
                                'y': {'field': 'index', 'type': 'nominal', 'title': ''},
                                'color': {
                                    'field': 'Correlation',
                                    'type': 'quantitative',
                                ...
                                'tooltip': [
                                    {'field': 'index', 'type': 'nominal', 'title': 'Metric 1'},
                                    {'field': 'Metric', 'type': 'nominal', 'title': 'Metric 2'},
                    ...
                    metric_cols = [c for c in rank_tbl.columns if c.startswith('Rank_')]
                    mcol = st.selectbox('Plot rank metric', metric_cols, index=0, key='sales rep_rank_metric')
                    plot = rank_tbl.dropna(subset=[mcol]).sort_values(mcol).head(25)
                    st.vega_lite_chart(
                        plot,
                        {
                            'width':'container',
                            ...
                            'mark':'bar',
                            'encoding':{
                                'x':{'field':'Sales Rep','type':'nominal','sort':'y','axis':{'labelAngle':-45}},
                                'y':{'field':mcol,'type':'quantitative','axis':{'title':mcol}},
                                'tooltip':[{'field':'Sales Rep','type':'nominal'},{'field':mcol,'type':'quantitative'}]
                            }
                        },
                        use_container_width=True
                    )
