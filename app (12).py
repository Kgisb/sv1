...
with st.sidebar:
    ...
    with st.expander("Filters", expanded=True):
        view_metric = st.radio(
            "View Metric",
            ["Total Revenue", "Total Enrollments", "AOV"],
        )
...
# Always compute Revenue + Enrollments
...
with tabs[0]:
    ...
    if date_col:
        ...
        trend_tbl = aggregate_metric_trend(df_f, date_col, primary_series, granularity)

        if not trend_tbl.empty:
            trend_tbl = trend_tbl.rename(columns={"metric": primary_label})
            st.line_chart(trend_tbl)

        if granularity == "Day":
            st.subheader(f"Weekday Split (%) – {view_metric}")
            wk = weekday_split(df_f, date_col, primary_series)
            if not wk.empty and wk["metric"].sum() > 0:
                ...
                st.dataframe(
                    ...
                    .rename(columns={"metric": primary_label, "pct_int": "Percentage"}),
                    use_container_width=True
                )
