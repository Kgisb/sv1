...
with tabs[0]:
    if mode == "Sales Rep":
        ...
        if bucket_selected_table is not None:
            ...
            st.dataframe(show[["Sales_Rep","Revenue_fmt","Enrollments_fmt","AOV_fmt"]].rename(columns={
                "Revenue_fmt":"Revenue",
                "Enrollments_fmt":"Enrollments",
                "AOV_fmt":"AOV"
            }), use_container_width=True)

            # NEW: Combined chart (X axis: Sales Rep)
            st.subheader("Sales Rep Comparison: Revenue (bar) + AOV & Enrollments (lines)")
            chart_df = bucket_selected_table.copy()
            chart_df = chart_df.replace([np.inf, -np.inf], np.nan).dropna(subset=["Revenue","AOV","Enrollments"])
            chart_df["Sales_Rep"] = chart_df["Sales_Rep"].astype(str)

            # Limit categories to keep chart readable
            max_reps = 30
            if len(chart_df) > max_reps:
                chart_df = chart_df.sort_values(rank_metric, ascending=False).head(max_reps)

            sort_order = chart_df.sort_values(rank_metric, ascending=False)["Sales_Rep"].tolist()
                    ...
                    "layer": [
                        {
                            ...
                            "encoding": {
                                "x": {"field": "Sales_Rep", "type": "nominal", "sort": sort_order, "axis": {"labelAngle": -45}},
                        ...
                        },
                        {
                            ...
                            "encoding": {
                                "x": {"field": "Sales_Rep", "type": "nominal", "sort": sort_order},
