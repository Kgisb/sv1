    ...
    if "Utm_Campaign" not in df.columns:
        ...
        if "Sales_Rep" not in utm_df.columns:
            ...
            if inquiry_id_col:
                ...
                st.caption("Rank 1 = best performer for that metric (within current filters/period).")
                ...
                rep_base["AOV"] = np.where(rep_base["Enrollments"] > 0, rep_base["Revenue"] / rep_base["Enrollments"], np.nan)
                ...
                # Avg conversion rate (proxy: enroll_series_fn(row) > 0) computed at lead level if LeadID exists
                ...
                # Productivity Index (if available in the app; else stays blank)
                ...
                rep_base["Rank (Revenue)"] = _rank_desc(rep_base["Revenue"])
                rep_base["Rank (Enrollments)"] = _rank_desc(rep_base["Enrollments"])
                rep_base["Rank (AOV)"] = _rank_desc(rep_base["AOV"])
                rep_base["Rank (Productivity)"] = _rank_desc(rep_base["Productivity_Index"]) if rep_base["Productivity_Index"].notna().any() else np.nan
                rep_base["Rank (Avg Calls)"] = _rank_desc(rep_base["Avg_Calls_Made"]) if rep_base["Avg_Calls_Made"].notna().any() else np.nan
                rep_base["Rank (Avg Conversion)"] = _rank_desc(rep_base["Avg_Conversion_%"])
                ...
                scoreboard = rep_base[[
                    ...
                    "Rank (Avg Calls)",
                    "Rank (Avg Conversion)",
                ]].copy().sort_values("Rank (Revenue)")

                st.dataframe(scoreboard.rename(columns={"Sales_Rep": "Sales Rep"}), use_container_width=True, height=520)

                st.divider()
                st.subheader("Graph View (Rank comparison)")
                metric_cols = [c for c in scoreboard.columns if c.startswith("Rank (")]
                y_col = st.selectbox("Select rank metric to plot", metric_cols, index=0, key="rep_rank_metric_plot")
                plot_df = scoreboard.dropna(subset=[y_col]).copy().sort_values(y_col).head(25)
                plot_df = plot_df.rename(columns={"Sales_Rep": "Sales Rep"})
                ...
                st.vega_lite_chart(
                    plot_df,
                    {
                        ...
                        "encoding": {
                            ...
                            "tooltip": [{"field": "Sales Rep", "type": "nominal"}, {"field": y_col, "type": "quantitative"}],
                        },
                    },
                    use_container_width=True,
                )
