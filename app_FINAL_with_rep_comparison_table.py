...
with tabs[0]:
    if mode != "Overall":
        ...
        # --- Comparative Sales Rep Scoreboard (Rankings) ---
        st.subheader("Sales Rep Comparative Scoreboard")
        st.caption(
            "Ranks are computed within the current filters/period. "
            "Lower rank number = better performance for that metric."
        )

        # Base rep aggregation (Revenue, Enrollments, AOV)
        rep_base = selected_table.rename(columns={"Group": "Sales_Rep"}).copy()
        rep_base["Revenue"] = pd.to_numeric(rep_base["Revenue"], errors="coerce").fillna(0)
        rep_base["Enrollments"] = pd.to_numeric(rep_base["Enrollments"], errors="coerce").fillna(0)
        rep_base["AOV"] = np.where(rep_base["Enrollments"] > 0, rep_base["Revenue"] / rep_base["Enrollments"], 0.0)

        # Avg calls made (uses Total_Dials if available)
        if "Total_Dials" in filtered_df.columns and "Sales_Rep" in filtered_df.columns:
            tmp_calls = filtered_df.copy()
            tmp_calls["Total_Dials"] = pd.to_numeric(tmp_calls["Total_Dials"], errors="coerce")
            calls_tbl = (
                tmp_calls.groupby("Sales_Rep")["Total_Dials"]
                .mean()
                .reset_index()
                .rename(columns={"Total_Dials": "Avg_Calls_Made"})
            )
            rep_base = rep_base.merge(calls_tbl, on="Sales_Rep", how="left")
        else:
            rep_base["Avg_Calls_Made"] = np.nan

        # Avg conversion rate (proxy: enroll_series_fn > 0 at row level)
        conv_tmp = filtered_df.copy()
        conv_flag = pd.to_numeric(enroll_series_fn(conv_tmp), errors="coerce").fillna(0)
        conv_tmp["_converted"] = (conv_flag > 0).astype(int)

        # Lead counting (prefer unique LeadID/InquiryID if present)
        lead_id_col = None
        for cand in ["LeadID", "LeadId", "Lead_ID", "leadid", "lead_id", "InquiryID", "Inquiry_Id", "ProspectID", "ProspectId"]:
            if cand in conv_tmp.columns:
                lead_id_col = cand
                break

        if "Sales_Rep" in conv_tmp.columns:
            if lead_id_col:
                lead_tbl = conv_tmp.groupby("Sales_Rep")[lead_id_col].nunique(dropna=True).reset_index().rename(columns={lead_id_col: "Leads"})
                conv_tbl = conv_tmp.groupby("Sales_Rep")["_converted"].sum().reset_index().rename(columns={"_converted": "Conversions"})
                conv_tbl = conv_tbl.merge(lead_tbl, on="Sales_Rep", how="left")
            else:
                conv_tbl = conv_tmp.groupby("Sales_Rep").agg(Leads=("Sales_Rep","size"), Conversions=("_converted","sum")).reset_index()

            conv_tbl["Avg_Conversion_%"] = np.where(conv_tbl["Leads"] > 0, (conv_tbl["Conversions"] / conv_tbl["Leads"]) * 100, 0.0)
            rep_base = rep_base.merge(conv_tbl[["Sales_Rep", "Avg_Conversion_%"]], on="Sales_Rep", how="left")
        else:
            rep_base["Avg_Conversion_%"] = np.nan

        # Productivity (if available from master_map + helper)
        rep_base["Productivity_Index"] = np.nan
        try:
            if "build_sales_rep_productivity" in globals():
                prod_df = build_sales_rep_productivity(filtered_df, master_map)
                if prod_df is not None and "Sales_Rep" in prod_df.columns and "Productivity_Index" in prod_df.columns:
                    rep_base = rep_base.merge(prod_df[["Sales_Rep", "Productivity_Index"]], on="Sales_Rep", how="left")
        except Exception:
            pass

        # Ranking helper (1 = best)
        def rank_desc(s):
            return s.rank(method="min", ascending=False).astype(int)

        rep_base["Rank_Revenue"] = rank_desc(rep_base["Revenue"])
        rep_base["Rank_Enrollments"] = rank_desc(rep_base["Enrollments"])
        rep_base["Rank_AOV"] = rank_desc(rep_base["AOV"])
        rep_base["Rank_Productivity"] = rank_desc(rep_base["Productivity_Index"]) if rep_base["Productivity_Index"].notna().any() else np.nan
        rep_base["Rank_Avg_Calls"] = rank_desc(rep_base["Avg_Calls_Made"]) if rep_base["Avg_Calls_Made"].notna().any() else np.nan
        rep_base["Rank_Conversion"] = rank_desc(rep_base["Avg_Conversion_%"]) if rep_base["Avg_Conversion_%"].notna().any() else np.nan
        ...
        show = rep_base[[
            ...
            "Rank_Avg_Calls",
            "Rank_Conversion",
        ]].sort_values("Rank_Revenue").rename(columns={
            "Sales_Rep": "Sales Rep",
            "Rank_Revenue": "Rank (Revenue)",
            "Rank_Enrollments": "Rank (Enrollments)",
            "Rank_AOV": "Rank (AOV)",
            "Rank_Productivity": "Rank (Productivity)",
            "Rank_Avg_Calls": "Rank (Avg Calls)",
            "Rank_Conversion": "Rank (Avg Conversion)",
        })

        st.dataframe(show, use_container_width=True, height=520)

        st.divider()
        st.subheader("Graph View (Rank comparison)")

        metric_cols = [c for c in show.columns if c.startswith("Rank (")]
        y_col = st.selectbox("Select rank metric to plot", metric_cols, index=0, key="rep_rank_metric_plot")

        plot_df = show.dropna(subset=[y_col]).copy().sort_values(y_col).head(25)
        ...
        st.vega_lite_chart(
            plot_df,
            {
                ...
                "mark": "bar",
                "encoding": {
                    "x": {"field": "Sales Rep", "type": "nominal", "sort": "y", "axis": {"labelAngle": -45, "title": "Sales Rep"}},
                    "y": {"field": y_col, "type": "quantitative", "axis": {"title": y_col}},
                    "tooltip": [
                        {"field": "Sales Rep", "type": "nominal"},
